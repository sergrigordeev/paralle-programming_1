...
public final class StudentAnalytics {
    ...
    public static final int GRADE_TRESHOLD = 65;
    public static final int AGE_THRESHOLD_20 = 20;
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        Optional<String> optName = Arrays.stream(studentArray).parallel()
    ...
    public int countNumberOfFailedStudentsOlderThan20Imperative(
        ...
        int count = 0;
        for (Student s : studentArray) {
            if (!s.checkIsCurrent() && s.getAge() > AGE_THRESHOLD_20 && s.getGrade() < GRADE_TRESHOLD) {
                count++;
            }
        }
        return count;
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        return (int) Arrays.stream(studentArray).parallel()
                .filter(e -> !e.checkIsCurrent() && e.getGrade() < GRADE_TRESHOLD)
                .filter(e -> e.getAge() > AGE_THRESHOLD_20)
                .count();
    }
}
